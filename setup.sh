...
# FIX: Store sensitive validation state files in the protected /root directory
UID_FILE="/root/bleat_uid.txt"
PODS_FILE="/root/bleat_pods.txt"
...
data:
  app.py: |
    ...
    def push_log(level, message):
        ...
        # FIX: Aggressive 40-retry loop ensures the app won't drop logs during the Loki rolling restart phase
        for _ in range(40):
            try:
