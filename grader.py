...
def get_running_pods():
    code, out, _ = run(f"kubectl get pods -n {BLEATER_NS} -l app={DEPLOY} -o jsonpath='{{range .items[*]}}{{.metadata.name}}{{\"\\n\"}}{{end}}'")
    if code != 0: return []
...
def check_live_configmap():
    ...
    try:
        ...
        # Checking for the specific implementation details (Mimics the report.json fail scenario)
